...
<article class="page-container">
	<form action="/index.php/Home/Orders/index.html" method="post" class="form form-horizontal submitform" id="form-member-add">
            {__TOKEN__} 
			...
			<div class="formControls col-xs-7 col-sm-4">
                            <input type="text" class="input-text receiver" value="<?php echo ($userInfo["name"]); ?>" placeholder="" name="receiver" id="receiver" errormsg="请输入中文名！"  nullmsg='请输入收货人姓名' datatype="*">
			...
			<div class="formControls col-xs-7 col-sm-4">
                            <input type="text" class="input-text mobile" value="<?php echo ($userInfo["mobile"]); ?>" placeholder="" name="mobile" id="post_code" errormsg="手机号码格式不正确！"  nullmsg='请输入手机号码' datatype="mobile">
			</div> <div class="Validform_checktip"></div>
		</div>
			...
			<div class="formControls col-xs-7 col-sm-4">
                            <!--value="<?php echo ($userInfo["post_code"]); ?>"-->
...
<!--                           -->
<input type="text"   value="<?php echo ($userInfo["detailed_address"]); ?>"  class="input-text detailed_address" placeholder="" name="detailed_address" id="detailed_address" errormsg="详细地址不正确！"  nullmsg='请输入详细地址' datatype="*">
			</div>
                      <div class="Validform_checktip"></div>
		</div>
            <div class="row cl">
		...
		<thead>
                ...
                <tbody>   
                <?php if(is_array($pro_list)): $i = 0; $__LIST__ = $pro_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pro_vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                <td>
                    <input type="checkbox" class="product_id" name="product_id" id="productSelect_<?php echo ($pro_vo["id"]); ?>"  value="<?php echo ($pro_vo["id"]); ?>"  onclick="onMemberProductSelectClick(this.checked,'<?php echo ($pro_vo["id"]); ?>')"></td>
               
                <td><?php echo ($pro_vo["product_title"]); ?></td>
                <td><?php echo ($pro_vo["member_price"]); ?>
                <input id="memberPrice_<?php echo ($pro_vo["id"]); ?>" type="hidden" value="<?php echo ($pro_vo["member_price"]); ?>">
                </td>
                <!--<td><?php echo ($pro_vo["product_pv"]); ?>-->
                <input id="memberPv_<?php echo ($pro_vo["id"]); ?>" type="hidden" value="<?php echo ($pro_vo["product_pv"]); ?>">
                </td>
                <td ><input type="type"  id="productNum_<?php echo ($pro_vo["id"]); ?>"  class='productNum_<?php echo ($pro_vo["id"]); ?>' maxlength="9" style="width:100px;" onblur="resetMemberProductAmount('<?php echo ($pro_vo["id"]); ?>',this.value)" onkeyup="clearNoInteger(event,this)"></td>
                <td><span id="productAmount_<?php echo ($pro_vo["id"]); ?>">0</span></td>
                <td > <a href="javascript:void(0);" onclick="showPage('700','650','商品详情','<?php echo U('Orders/productInfo',array('id'=>$pro_vo['id']));?>')"  >查看商品详情</a></td>
      
                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                
                <?php if($flag == 1 ): ?><tr> 
                            <td colspan="6"> <span onclick="gwjf()" style="cursor: pointer;color:#0331da;">使用购物积分 </span> <div id="gwjf" style="display: none"><input  type="text" class="input-text gwjf"  onblur="verifygwjf(<?php echo ($userInfo["gouwujifen"]); ?>)"  style="width:150px;" ><span>剩余购物积分：<?php echo ($userInfo["gouwujifen"]); ?></span></div></td>
                     </tr> 
                     <td colspan="6"><span  onclick="gwj()" style="cursor: pointer;color:#0331da;">使用购物卷</span> <div id="gwj" style="display: none;"> <input  type="text" class="input-text gwj"   style="width:150px;" onblur="verifygwj(<?php echo ($userInfo["gouwujuan"]); ?>)" >  剩余购物卷：<?php echo ($userInfo["gouwujuan"]); ?>，可用购物卷：<span id="aa" style="font-weight: 900;color:green;font-size: 20px;">0</span>  </div></td>
                </tr>
                <tr> 
                    <td colspan="6"><span onclick="xjjf()" style="cursor: pointer;color:#0331da;">使用现金积分</span> <div id="xjjf" style="display: none;"><input  type="text" class="input-text xjjf"   style="width:150px;" onblur="verifyxjjf(<?php echo ($userInfo["cash"]); ?>)" >  剩余现金积分：<?php echo ($userInfo["cash"]); ?></div></td>
                     </tr>
                     <tr>
                         <td colspan="6"><span onclick="dzjf()" style="cursor: pointer;color:#0331da;">使用电子积分</span><div id="dzjf" style="display: none;"> <input  type="text" class="input-text dzjf"    style="width:150px;" onblur="verifdzjf(<?php echo ($userInfo["dianzimoney"]); ?>)" >  剩余电子币：<?php echo ($userInfo["dianzimoney"]); ?></div></td>
                     </tr>
                 <?php else: ?>
                 <tr> 
                            <td colspan="6"> <span onclick="gwjf()" style="cursor: pointer;color:#0331da;">使用购物积分 </span> <div id="gwjf" style="display: none"><input  type="text" class="input-text gwjf"  onblur="verifygwjf(<?php echo ($userInfo["gouwujifen"]); ?>)"  style="width:150px;" ><span>剩余购物积分：<?php echo ($userInfo["gouwujifen"]); ?></span></div></td>
...
//转换对象，使用递归，逐层剥到文本
    ...
    function getCookies(Name)            //cookies读取
    {
        var search = Name + "="
...
function resetMemberProductAmount(productId, num) {
	...
	$("#productTotalAmount").text(totalProductAmount);
        if(<?php echo ($userInfo["gouwujuan"]); ?><totalProductAmount/2){
            $("#aa").text(<?php echo ($userInfo["gouwujuan"]); ?>);
        }
        else{
